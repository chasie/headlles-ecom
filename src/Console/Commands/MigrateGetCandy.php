...
class MigrateGetCandy extends Command
{
     ...
     * @var string
     */
    protected $signature = 'headless-ecom:migrate:getcandy';
     ...
     * @var string
     */
    protected $description = 'Migrate GetCandy into HeadlessEcom';
    ...
    public function handle()
    {
        ...
        $HeadlessEcomTables = $tableNames->filter(function ($table) {
            return str_contains($table, 'headless-ecom_');
        });

        if ($tables->count() && ! $HeadlessEcomTables->count()) {
            ...
            DB::transaction(function () use ($table, $rows) {
                foreach ($rows as $row) {
                    DB::table($table)->update([
                        $row => DB::RAW(
                            "REPLACE({$row}, 'GetCandy', 'headless-ecom')"
                        ),
                    ]);
                }
            });
        ...
        foreach ($tables as $table) {
            ...
            DB::table($tableName)->update([
                'attribute_data' => DB::RAW(
                    "REPLACE(attribute_data, 'GetCandy', 'headless-ecom')"
                ),
            ]);
        }
    ...
    protected function migrateTableNames($tables)
    {
        ...
        foreach ($tables as $table) {
            $old = $table;
            $new = str_replace('getcandy_', 'headless-ecom_', $table);
            ...
            if ($old == 'getcandy_products') {
                if (Schema::hasColumn('getcandy_products', 'brand')) {
                    ...
                    DB::table('headless-ecom_brands')->insert(
            ...
            $brands = DB::table('headless-ecom_brands')->get();
