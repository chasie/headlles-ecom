...
class Manifest extends PackageManifest
{
    ...
    public function build()
    {
        ...
        $this->write(collect($packages)->filter(function ($package) {
            return Arr::has($package, 'extra.HeadlessEcom');
    ...
    protected function formatPackage($package)
    {
        ...
        $HeadlessEcom = $json['extra']['headless-ecom'] ?? [];
        $author = $json['authors'][0] ?? null;

        $config = config('headless-ecom.addons.'.$package['name'], [
            'license' => null,
        ]);

        return [
            'id' => base64_encode($package['name']),
            'slug' => $HeadlessEcom['slug'] ?? null,
            'editions' => $HeadlessEcom['editions'] ?? [],
            'github_url' => $package['source']['url'],
            'version' => $package['version'],
            'namespace' => $namespace,
            'autoload' => $autoload,
            'provider' => $provider,
            'name' => $HeadlessEcom['name'] ?? Arr::last($providerParts),
