...
namespace HeadlessEcom\Listeners;
...
class CartSessionAuthListener
{
    ...
    {
        if (! is_HeadlessEcom_user($event->user)) {
            return;
        }
        ...
        if ($currentCart && ! $currentCart->user_id) {
            CartSession::associate(
                $currentCart,
                $event->user,
                config('headless-ecom.cart.auth_policy')
            );
        }
    ...
    {
        if (is_null($event->user) || ! is_HeadlessEcom_user($event->user)) {
            return;
        }
